...
void Encoder_Init(void)
{
	...
	/*AFIO选择中断引脚*/
	GPIO_EXTILineConfig(GPIO_PortSourceGPIOB, GPIO_PinSource3);//将外部中断的0号线映射到GPIOB，即选择PB0为外部中断引脚
	GPIO_EXTILineConfig(GPIO_PortSourceGPIOB, GPIO_PinSource4);//将外部中断的1号线映射到GPIOB，即选择PB1为外部中断引脚
	
	/*EXTI初始化*/
	EXTI_InitTypeDef EXTI_InitStructure;						//定义结构体变量
	EXTI_InitStructure.EXTI_Line = EXTI_Line3 | EXTI_Line4;		//选择配置外部中断的0号线和1号线
	...
	NVIC_InitStructure.NVIC_IRQChannel = EXTI3_IRQn;			//选择配置NVIC的EXTI0线
