...
void Redraw_Mainmenu(void)
{
	...
	Gui_DrawFont_GBK16(8,60,BLUE,GRAY0,"文字显示");

	DisplayButtonUp(0,98,80,118); //x1,y1,x2,y2
	Gui_DrawFont_GBK16(80,20,RED,GRAY0,"图片显示");
	Delay_ms(1500);
}
...
void Font_Test(void)
{
	...
	Delay_ms(1000);
	Lcd_Clear(GRAY0);
	Gui_DrawFont_GBK16(8,0,RED,GRAY0,"五邑大学");
...
//取模方式 水平扫描 从左到右 低位在前
void showimage(const unsigned char *p) //显示64*64 QQ图片
{
  	int i,j,k; 
	...
	for(k=0;k<2;k++)
	{
	   	for(j=0;j<4;j++)
		{	
			Lcd_SetRegion(64*j,64*k,64*j+63,64*k+63);		//坐标设置
		    for(i=0;i<64*64;i++)
			 {	
			 	picL=*(p+i*2);	//数据低位在前
				picH=*(p+i*2+1);				
